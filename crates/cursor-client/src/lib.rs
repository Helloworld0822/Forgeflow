use autoforge_shared::{AgentMode, AutoForgeError, ModelProfile, Result};
use reqwest::Client;
...
struct CreateAgentRequest<'a> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    mcp_servers: Option<Vec<McpServer<'a>>>,
}
...
impl CursorClient {
    ...
    pub async fn create_agent(
        ...
        let model = ModelSelection {
            id: &profile.model_id,
            params: if profile.params.is_empty() {
                None
            } else {
                Some(
                    profile
...
pub struct CreateAgentOpts<'a> {
    ...
    pub mcp_servers: Option<Vec<McpServer<'a>>>,
}

impl<'a> Default for CreateAgentOpts<'a> {
    fn default() -> Self {
        Self {
            repo_url: None,
            starting_ref: None,

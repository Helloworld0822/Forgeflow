...
use autoforge_artifacts::ArtifactStore;
use autoforge_cursor_client::{CreateAgentOpts, CursorClient};
use autoforge_ingest::ingest_pdf;
use autoforge_shared::{
    ArtifactRef, AutoForgeError, ModelProfile, Result, StageCommand, StageId,
};
use autoforge_stitch_client::StitchClient;
use bytes::Bytes;
use std::sync::Arc;
...
impl StageExecutor for DesignExecutor {
    ...
    async fn execute(&self, ctx: &StageContext) -> Result<StageOutput> {
        ...
        let _base = format!("projects/{}/design", ctx.command.project_id.0);
...
pub fn all_executors(repo_url: String) -> Vec<Arc<dyn StageExecutor>> {
    vec![
        Arc::new(IngestExecutor),
        Arc::new(SummarizeExecutor),
        Arc::new(ArchitectExecutor),
        Arc::new(DesignExecutor),
        Arc::new(ImplementExecutor { repo_url }),
    ]
}
